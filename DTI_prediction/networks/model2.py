...
class SPD_DTI(nn.Module):
    ...
    def forward(self, inputs):
        ...
        DTI_Feature1_ = self.DTI_SPD(DTI_Feature1)
        DTI_Feature2_ = self.DTI_SPD2(DTI_Feature2)
        DTI, dti_feature = self.classfier(DTI_Feature1_, DTI_Feature2_)
        return DTI, ancillary, DTI_Feature1_, self.covariance(Joint_Feature)
    ...
    def __call__(self, data, epoch=1, train=True):
        ...
        protein_drug_interaction, ancillary, DTI_Feature1, DTI_Feature2 = self.forward(inputs)  # , dis_invariant
        t = int(epoch / 2)
        alpha = 0.5 * (10 / (t + 10))
        if train:
            ...
            return correct_labels, ys, DTI_Feature1, DTI_Feature2