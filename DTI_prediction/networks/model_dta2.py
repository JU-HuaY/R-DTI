...
class Ancillary_LeNet(nn.Module):
    def __init__(self, hidden, classes, layers):
        ...
        self.FC_out = nn.Linear(hidden, classes)
        self.layers = layers
        self.act = nn.ReLU()
...
class SPD_DTA(nn.Module):
    def __init__(self, layer_gnn, device, hidden1=192, hidden2=64, n_layers=3, dropout=0.05):
        ...
        self.a_c = Ancillary_LeNet(hidden1 + hidden2, 1, n_layers)
        ...
        self.DTI_SPD2 = SPD(16, 1e-6)
        self.device = device
        self.layer_gnn = layer_gnn
        self.classfier = SPD_LeNet(32, 16, 1, n_layers)
        self.act = nn.ReLU()
        self.GELU = nn.GELU()
    ...
    def forward(self, inputs):
        ...
        return DTI.squeeze(1), ancillary.squeeze(1)

    def __call__(self, data, epoch=1, train=True):
        inputs, correct_interaction, SID = data[:-2], data[-2], data[-1]
        correct_interaction = torch.FloatTensor(correct_interaction.to('cpu').numpy()).cuda()
        protein_drug_interaction, ancillary = self.forward(inputs)  # , dis_invariant
        # interactions = a2i(correct_interaction, "Davis")
        # SL1_loss = nn.SmoothL1Loss()
        MSE_loss = nn.MSELoss()
        t = int(epoch / 2)
        alpha = 0.5 * (10 / (t + 10))
        if train:
            loss = MSE_loss(protein_drug_interaction, correct_interaction)
            loss2 = MSE_loss(ancillary, correct_interaction)
            return loss * (0.9 - alpha) + loss2 * (0.1 + alpha)
        else:
            correct_labels = correct_interaction  # .to('cpu').data.numpy().reshape(-1)
            ys = protein_drug_interaction  # .to('cpu').data.numpy() C
