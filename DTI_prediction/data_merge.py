...
def val_data_load(dataset, device, DTI=True): # BindingDB
    molecule_words_test = load_tensor(dataset + '/validation/molecule_words', torch.LongTensor, device)
    molecule_atoms_test = load_tensor(dataset + '/validation/molecule_atoms', torch.LongTensor, device)
    molecule_adjs_test = load_tensor(dataset + '/validation/molecule_adjs', torch.LongTensor, device)
    proteins_test = load_tensor(dataset + '/validation/proteins', torch.LongTensor, device)
    sequence_test = np.load(dataset + '/validation/sequences.npy')
    smiles_test = np.load(dataset + '/validation/smiles.npy')
    if DTI == True:
        interactions_test = load_tensor(dataset + '/validation/interactions', torch.LongTensor, device)
    else:
        interactions_test = load_tensor(dataset + '/validation/affinities', torch.FloatTensor, device)

    with open(dataset + '/validation/p_LM.pkl', 'rb') as p:
        p_LM = pickle.load(p)

    with open(dataset + '/validation/p_LM_SPD.pkl', 'rb') as p:
        p_SPDs = pickle.load(p)

    with open(dataset + '/validation/d_LM.pkl', 'rb') as d:
        d_LM = pickle.load(d)
...
def merge2(train_list, val_list, device, DTI=True):
    molecule_words_trains, molecule_atoms_trains, molecule_adjs_trains, proteins_trains, sequence_trains, smiles_trains, interactions_trains = [], [], [], [], [], [], []
    ...
    for dataset in val_list:
        molecule_words_val, molecule_atoms_val, molecule_adjs_val, proteins_val, sequence_val, smiles_val, p_LM, p_SPD, d_LM, interactions_val = val_data_load(dataset, device, DTI)
    ...
    return train_dataset, val_dataset, p_LMs, p_SPDs, d_LMs

def data_load(data_select, device):
    # p_LMs, d_LMs = {}, {}
    if data_select == "B_to_B":
        train_list = ["datasets/BindingDB"]
        val_list = ["datasets/BindingDB"]
        train_dataset, val_dataset, p_LMs, p_SPDs, d_LMs = merge2(train_list, val_list, device)
    elif data_select == "B_to_B_test":
        train_list = ["datasets/BindingDB"]
        test_list = ["datasets/BindingDB"]
        train_dataset, val_dataset, p_LMs, p_SPDs, d_LMs = merge(train_list, test_list, device)
