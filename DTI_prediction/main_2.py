...
from networks.model2 import SPD_DTI
    ...
    def test(self, dataset, p_LMs, p_SPDs, d_LMs):
        ...
        for data in dataset:
            ...
            if i % self.batch_size == 0 or i == N:
                ...
                correct_labels, ys, DTI_Feature1, DTI_Feature2 = self.model(data, train=False)
                correct_labels = correct_labels.to('cpu').data.numpy()
                DTI_Feature1 = DTI_Feature1.to('cpu').data.numpy()
                DTI_Feature2 = DTI_Feature2.to('cpu').data.numpy()
                ys = ys.to('cpu').data.numpy()
                predicted_labels = list(map(lambda x: np.argmax(x), ys))
                predicted_scores = list(map(lambda x: x[1], ys))

                for j in range(len(correct_labels)):
                    if len(DTI_Feature2s) > 100:
                        continue
                    T.append(correct_labels[j])
                    Y.append(predicted_labels[j])
                    S.append(predicted_scores[j])
                    DTI_Feature1s.append(DTI_Feature1[j])
                    DTI_Feature2s.append(DTI_Feature2[j])
        ...
        PRC = auc(fpr, tpr)
        return AUC, PRC, precision, recall, DTI_Feature1s, DTI_Feature2s
...
if __name__ == "__main__":
    ...
    data_select = "B_to_B_fine" # D_to_D SPD(32, 1e-3)
    iteration = 2
    decay_interval = 6
    batch_size = 16
    lr = 1e-5
    ...
    for epoch in range(1, iteration):
        ...
        AUC, PRC, precision, recall, DTI_Feature1s, DTI_Feature2s = tester.test(dataset_test, p_LMs, p_SPDs, d_LMs)

        end = timeit.default_timer()
        time = end - start
        np.save("DTI_Feature1s.npy", DTI_Feature1s)
        np.save("DTI_Feature2s2.npy", DTI_Feature2s)
        AUCs = [epoch, time, time,
                AUC, PRC, precision, recall]
