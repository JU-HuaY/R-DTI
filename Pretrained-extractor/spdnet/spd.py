...
class SPDTransform(nn.Module):
    ...
    def forward(self, input):
        ...
        weight = weight.expand(input.size(0), input.size(1), -1, -1)
        output = torch.matmul(weight.transpose(2,3), torch.matmul(output, weight))
        return output
...
class SPDTangentSpaceFunction(Function):
    ...
    def forward(ctx, input):
        ...
        output = input.new(input.size(0), input.size(1), input.size(2), input.size(3))
        for b, x0 in enumerate(input):
            for k, x in enumerate(x0):
                u, s, v = x.svd()
                s.log_()
                output[b][k] = u.mm(s.diag().mm(u.t()))
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            eye = input.new(input.size(2))
            eye.fill_(1); eye = eye.diag()
            grad_input = input.new(input.size(0), input.size(1), input.size(2), input.size(2))
            for b, g0 in enumerate(grad_output):
                for k, g in enumerate(g0):
                    x = input[b][k]
                    u, s, v = x.svd()
                    ...
                    P = 1 / P
                    P[mask_zero] = 0
                    grad_input[b][k] = u.mm(symmetric(P.t() * (u.t().mm(dLdV)))+dLdS).mm(u.t())
...
class SPDRectifiedFunction(Function): #4D
...
class SPDRectifiedFunction(Function): # 4D
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            eye = input.new(input.size(2))
            eye.fill_(1); eye = eye.diag()
            grad_input = input.new(input.size(0), input.size(1), input.size(2), input.size(3))
            for b, g0 in enumerate(grad_output):
                for k, g in enumerate(g0):
                    if len(g.shape) == 1:
                        continue
...
Huang, Z., & Van Gool, L. J. (2017, February). A Riemannian Network for SPD Matrix Learning. In AAAI (Vol. 1, No. 2, p. 3).
"""
class SPDRectified(nn.Module): # 4D
...
class GrassmannianMapping(nn.Module):
    def __init__(self, feature_dim, subspace_dim):
        super(GrassmannianMapping, self).__init__()
        self.projection = nn.Linear(feature_dim, subspace_dim, bias=False)
    ...
    def forward(self, x):
        ...
        # 计算 SVD，得到基向量
        U, _, _ = torch.svd(projected_x, some=True)
        # 选取前 k 个基向量，构成 Grassmannian 上的点
        grassmannian_point = U[:, :self.subspace_dim]

        return grassmannian_point

class StiefelMapping(nn.Module):

    def __init__(self, feature_dim, subspace_dim):
        super(StiefelMapping, self).__init__()
        self.projection = nn.Linear(feature_dim, subspace_dim, bias=False)

    def forward(self, x):
        # 映射到子空间
        projected_x = self.projection(x)
        # 对投影后的特征进行 QR 分解，得到正交矩阵 Q
        Q, _ = torch.qr(projected_x)
        # 返回 Stiefel manifold 上的点
        stiefel_point = Q

        return stiefel_point