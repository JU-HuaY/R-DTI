...
class SPDMLP(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, hidden_dim, num_layer=2):
        ...
        for layer_idx in range(num_layer):
            if layer_idx == 0:
                layers.append(SPDTransform(in_dim, hidden_dim))
            elif layer_idx == num_layer - 1:
                layers.append(SPDTransform(hidden_dim, out_dim))
            else:
                layers.append(SPDTransform(hidden_dim, hidden_dim))
